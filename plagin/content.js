(() => {
    ...
    function convertPostDate(input) {
        ...
        if (input.includes('только что')) return now;
        ...
        if (relativeMatch) {
            ...
            switch (unit) {
                case 'мин': diff = value * 60 * 1000; break;
                case 'ч': diff = value * 60 * 60 * 1000; break;
                case 'д': diff = value * 24 * 60 * 60 * 1000; break;
                case 'н': diff = value * 7 * 24 * 60 * 60 * 1000; break;
            }
        ...
        if (absoluteMatch) {
            const [, day, monthStr, year = now.getFullYear()] = absoluteMatch;
            const months = {
